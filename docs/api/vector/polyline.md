...
![Polyline 示意图 1](/img/refdoc/api/polyline.gif)
...
- **适用行业**：智慧城市、交通、智慧水利、能源、应急、园区、测绘、国防、海洋。、低空经济、智慧工地、石油化工、地下综合管线、人防应急。
- **使用场景**：
  - 道路/轨道/河道等线状基础设施的路网与中心线展示。
  - 给排水、燃气、电力、热力等市政管线的走向与流向（光流样式）表达。
  - 车辆/船舶/人员的历史与实时运动轨迹绘制。
  - 三维航线绘制：连续打点生成航线 / 航道带，配合空域立体范围展示低空飞行通道。
  - 巡检轨迹回放、集输 / 地下管线走向、疏散与救援处置流线绘制。
- **注意事项**：
...
![Polyline · add(data, fn) 示意](/img/refdoc/api/copy_material_path.gif)
...
| `arcType` | `number` | 否 | 0 | 球面地形下绘制贴地弧线的类型，0：劣弧 1：优弧，默认值：0 |
...
| `thickness` | `number` | 否 | 20 | 线宽，单位：米，默认值20 |
| `intensity` | `number` | 否 | 0.5 | 亮度，取值范围：[0~1000]，默认值：0.5 |
| `flowRate` | `number` | 否 | 0.5 | 流速，取值范围：[0~1.0]，默认值：0.5 |
| `depthTest` | `boolean` | 是 | - | 是否做深度检测，默认为true，true会被遮挡，false不会被遮挡 |
| `shape` | `number` | 是 | - | 样式，0：直线， 1：曲线，注意：设置为曲线坐标点多的时候会影响添加添加效率 |
| `style` | [`PolylineStyle`](/docs/api/types#polylinestyle) | 是 | - | (`PolylineStyle`) 折线样式，箭头/光流/贴地/实线/虚线等，取值范围：[0~7]，详情参考 `PolylineStyle` |
| `tiling` | `number` | 否 | - | 可选参数，材质贴图平铺比例，和PolylineStyle取值有关，目前仅部分样式支持此参数，从起始位置开始平铺，超过的部分会按此比例生成新的区域，类似CSS的repeat。如果这个值 &lt;= 0 使用自动计算按Polyline长度比例平铺， &gt;0使用用户输入的值去平铺 |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `data` | `object \| array` | 是 | - | Polyline对象id和模型对象Id的数据映射对象数组，可以是Object类型或者Array类型，对于每一个映射对象，支持以下属性： |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 属性 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `polylineId` | `string` | 是 | - | Polyline对象ID |
| `startObjectId` | `string` | 是 | - | Polyline起点跟随移动对应的模型ID |
| `endObjectId` | `string` | 是 | - | Polyline终点跟随移动对应的模型ID |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | 要删除的Polyline对象的ID或者ID数组（可以删除一个或者多个） |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | Polyline对象的ID或者ID数组 |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | 要获取的Polyline对象ID或者ID数组（可以获取一个或者多个） |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
|------|------|:----:|--------|------|
| `id` | `string` | 是 | - | 字符串类型的ID |
| `newVal` | `number` | 是 | - | PolylineStyle新样式，取值范围：[0~5]，参考 `PolylineStyle` |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
