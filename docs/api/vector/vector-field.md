...
description: "向量场类对象，基于真实数据对风场、波浪、洋流、河流、箭头、烟雾的场数据形态进行仿真，支持tif和bin格式 一般通过api.vectorField调用其方法 数据格式参考以下说明： 流场数据最终提供给程序使用是以三维纹理的方式用于渲染，所以要求在三个维度上采样均匀，也就是采样点间隔均匀，否则会变形甚"
---
