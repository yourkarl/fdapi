...
- **适用行业**：国防军事、应急指挥、公安警务、边海防、城市治理、智慧交通与公路
- **使用场景**：
  - 作战或应急指挥的态势图标绘与力量部署
  - 行动路线、警戒线、责任区标注
  - 指挥所推演与态势图层构建
  - 工程测量标绘：距离 / 面积 / 高度等标绘测量，结果经 Measurement 事件回传，用于公路监管量算。
- **注意事项**：
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `data` | `object \| array` | 是 | - | Plot对象或对象数组，对于每一个对象，支持以下属性： |
...
| 属性 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `id` | `string` | 是 | - | Plot对象唯一标识符ID |
| `groupId` | `string` | 否 | - | 可选，Group分组 |
| `userData` | `string` | 否 | - | 可选，用户自定义数据 |
| `style` | [`PlotStyle`](/docs/api/types#plotstyle) | 是 | - | (`PlotStyle`) 态势标绘的图形类型，取值范围：[0~16]，详情参考 `PlotStyle` |
| `range` | `array` | 是 | - | 可视范围: [近裁距离, 远裁距离]，取值范围: [任意负值, 任意正值] |
| `coordinates` | `array` | 是 | - | 标绘区域包含锚点对应的坐标点数组，[取值示例](/docs/tutorials/coordinates) |
| `coordinateType` | `number` | 否 | 0 | 坐标系类型，取值范围：0为Projection类型，1为WGS84类型，2为火星坐标系(GCJ02)，3为百度坐标系(BD09)，默认值：0 |
| `lineColor` | [`Color`](/docs/api/types#color) | 否 | 白色 | 边框线的颜色，默认值：白色，支持四种格式，[取值示例](/docs/tutorials/color) |
| `fillColor` | [`Color`](/docs/api/types#color) | 否 | 红色 | 填充区域颜色，默认值：红色，支持四种格式，[取值示例](/docs/tutorials/color) |
| `thickness` | `number` | 否 | 3px | 线宽，默认值：3px，单位：像素，注意：仅部分样式下生效 |
| `onTerrain` | `boolean` | 否 | true | 可选，绘制后是否贴地，默认值：true |
| `depthTest` | `boolean` | 否 | true | 可选，是否做深度检测，默认值：true，true会被遮挡，false不会被遮挡 |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | 要删除的Plot对象的ID或者ID数组（可以删除一个或者多个） |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | Plot对象的ID或者ID数组 |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | 要获取的Plot对象ID或者ID数组（可以获取一个或者多个） |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `plot` | `object` | 是 | - | 待绘制的Plot对象，支持以下属性： |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `data` | `object \| array` | 是 | - | 对象数据结构，结构参考add()方法 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
