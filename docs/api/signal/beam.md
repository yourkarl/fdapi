...
![Beam 示意图](/img/refdoc/api/Beam.Add.gif)
...
- **适用行业**：国防雷达、通信、低空监管、安防、航空管制、低空经济与通信
- **使用场景**：
  - 雷达探测范围与扫描动态的展示
  - 通信波束指向与覆盖
  - 低空目标探测态势
  - 低空通信与雷达可视化：雷达扇形 / 通信波束渲染，表达探测 / 覆盖范围，配合建筑遮挡分析。
- **注意事项**：
...
| `duration` | `number` | 是 | - | 光流粒子的生命周期，取值范围：[0.1~3.0]，单位：秒 |
| `thickness` | `number` | 是 | - | 光流线的宽度，取值范围： [0.01~10.0]，单位：因Beam是粒子加上自发光不能精确单位,故目前按比例显示,需要精确单位的推荐使用Polyline |
| `interval` | `number` | 是 | - | 光流粒子发射间隔，取值范围：[1.0~10.0]，单位：秒 |
| `velocity` | `number` | 是 | - | 光流粒子的速度，取值范围：[0.1~5.0] |
| `color` | [`Color`](/docs/api/types#color) | 是 | - | 光流的颜色，支持四种格式，[取值示例](/docs/tutorials/color) |
| `coordinates` | `array` | 是 | - | 光流的polyline的坐标数组 |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | 要删除的Beam对象的ID或者ID数组（可以删除一个或者多个） |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | Beam对象的ID或者ID数组 |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | 要获取的Beam对象ID或者ID数组（可以获取一个或者多个） |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
```js
await fdapi.beam.setColor('对象ID', '示例值');
```
...
```js
await fdapi.beam.setCoordinates('对象ID', []);
```
...
```js
await fdapi.beam.setDuration('对象ID', '示例值');
```
...
```js
await fdapi.beam.setInterval('对象ID', 0);
```
...
|------|------|:----:|--------|------|
| `id` | `string` | 是 | - | Beam对象的ID |
| `newVal` | `number` | 是 | - | 新厚度值，取值范围：[0.01-100] |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
```js
await fdapi.beam.setVelocity('对象ID', 0);
```
