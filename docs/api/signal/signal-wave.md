...
- **适用行业**：通信、国防、物联网、应急通信、广电、低空经济与通信
- **使用场景**：
  - 信号源覆盖扩散的动态表达
  - 通信/广播传播示意
  - 事件信号传播的可视化
  - 低空通信信号覆盖：信号扩散覆盖渐变色块，自动镂空建筑遮挡盲区，支撑通信基站选址。
- **注意事项**：
  - 为示意性可视化，非精确传播模型
  - 扩散参数需结合业务设定
  - 与 Antenna、Beam 配合使用
...
| `alpha` | `number` | 是 | - | 波束的透明度，取值范围：[0~1] |
| `range` | `array` | 是 | - | 距离波束发射点A的衰减范围，[min,max]，在min和max之间显示透明度渐变，大于max则波束消失。 |
| `valA` | `array` | 是 | - | 构造波束的位置坐标A: [x, y, z]，[取值示例](/docs/tutorials/coordinates) |
| `valB` | `array` | 是 | - | 构造波束的位置坐标B: [x, y, z]，[取值示例](/docs/tutorials/coordinates) |
| `valC` | `array` | 是 | - | 构造波束的位置坐标C: [x, y, z]，[取值示例](/docs/tutorials/coordinates) |
| `valD` | `array` | 是 | - | 构造波束的位置坐标D: [x, y, z]，[取值示例](/docs/tutorials/coordinates) |
| `valE` | `array` | 是 | - | 构造波束的位置坐标E: [x, y, z]，[取值示例](/docs/tutorials/coordinates) |
| `valF` | `array` | 是 | - | 构造波束的位置坐标F: [x, y, z]，[取值示例](/docs/tutorials/coordinates) |
| `valG` | `array` | 是 | - | 构造波束的位置坐标G: [x, y, z]，[取值示例](/docs/tutorials/coordinates) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | 要删除的SignalWave对象的ID或者ID数组（可以删除一个或者多个） |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | SignalWave对象的ID或者ID数组 |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | 要获取的SignalWave对象ID或者ID数组（可以获取一个或者多个） |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
