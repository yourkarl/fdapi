...
![GeoJSONLayer 示意图](/img/refdoc/api/geojson.gif)
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `option` | `object` | 是 | - | 加载GeoJSON的配置项对象，包含以下属性 |
...
| 属性 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `id` | `string` | 是 | - | 必选，字符串类型唯一id |
| `url` | `string` | 是 | - | 二选一，GeoJSON文件本地磁盘路径或者对应的url地址 |
| `sourceJson` | `Object` | 是 | - | 二选一，GeoJSON标准对象（JsonObject） |
| `visible` | `boolean` | 否 | - | 可选，设置图层加载后是否显示，默认：true |
| `rotation` | `array` | 是 | - | 旋转：[Pitch,Yaw,Roll]，数组元素类型：(number)，取值范围：[任意数值] |
| `offset` | `array` | 否 | [0,0,0] | 可选，基于原始位置的偏移量，默认值：[0,0,0] |
| `range` | `array` | 否 | [0, 10000] | 点的可视范围: [近裁距离, 远裁距离]，默认值: [0, 10000]，注意：此参数仅对point类型的GeoJSON生效 |
| `viewHeightRange` | `array` | 否 | [-1000000000, 1000000000] | 可见高度范围：[最小可见高度, 最大可见高度]，默认值: [-1000000000, 1000000000] |
| `needProject` | `boolean` | 否 | - | 可选，是否进行投影转换，默认：true |
| `textField` | `string` | 否 | - | 可选，使用Geojson的属性字段显示文字标注 |
| `textRange` | `array` | 否 | - | 可选，文字标注的可视范围: [近裁距离, 远裁距离]，单位：米 |
| `textSize` | `number` | 否 | - | 可选，文字标注的字体大小，默认大小：24 |
| `textColor` | [`Color`](/docs/api/types#color) | 否 | - | 可选，文字标注的字体颜色，默认颜色：[1,1,1,1] |
| `onTerrain` | `boolean` | 否 | - | 可选，设置图层加载后是否贴地，默认：false |
| `collision` | `boolean` | 否 | true | 是否开启碰撞，默认值：true |
| `depthTest` | `boolean` | 是 | - | 是否做深度检测，默认开启：true，true会被地形遮挡，false不会被地形遮挡，注意：深度检测参数仅在反走样参数关闭时(enableAntialias=false)生效 |
| `enableAntialias` | `boolean` | 是 | - | 是否开启反走样(仅对polyline类型生效)，默认开启：true |
| `renderer` | `object` | 是 | - | 必选，渲染器配置对象参数如下： **注意：使用此自定义材质需单独按规范打包，具体参考相关教程。** |
| `renderer.rendererType` | [`RendererType`](/docs/api/types#renderertype) | 是 | - | (`RendererType`) 渲染器类型，包含三种：简单渲染器、唯一值渲染器、分类渲染器 |
| `renderer.generateTop` | `boolean` | 否 | - | 可选参数，是否生成顶面，默认：true |
| `renderer.generateBottom` | `boolean` | 否 | - | 可选参数，是否生成底面，默认：true |
| `renderer.style` | `-` | 是 | - | `Polygon3DStyle` 仅对Polygon3D渲染生效，设置Polygon3D的样式 |
| `renderer.type` | [`VisualType`](/docs/api/types#visualtype) | 是 | - | (`VisualType`) 控制显示的可视化类型枚举 |
| `renderer.field` | `string` | 是 | - | GeoJSON文件内的属性字段名称 |
| `renderer.fieldType` | [`FieldType`](/docs/api/types#fieldtype) | 是 | - | (`FieldType`) GeoJSON文件内的属性字段类型，枚举支持数值类型和字符串类型 |
| `renderer.gradient` | `boolean` | 是 | - | 颜色是否渐变 |
| `renderer.materials` | `array` | 否 | - | 可选，自定义材质配置列表，每一个材质对象包含的参数如下： |
| `renderer.index` | `number` | 是 | - | 自定义材质索引 |
| `renderer.material` | `string` | 是 | - | 自定义材质路径，即资源库PAK文件里材质文件的路径，设置自定义材质参数后style相关参数会失效 |
| `renderer.scalarParameters` | `array` | 否 | - | 可选参数，仅在设置自定义材质路径后生效，自定义材质数值类型参数，包含name/value键值对的数组，其中value为数值，格式示例：[&#123;"name":"不透明度","value":0.5&#125;,&#123;"name":"UV重复","value":1.0&#125;] |
| `renderer.vectorParameters` | `array` | 否 | - | 可选参数，仅在设置自定义材质路径后生效，自定义材质数组类型参数，包含name/value键值对的数组，其中value为数组，格式示例：[&#123;"name":"color1","value":[1,1,1,1]&#125;,&#123;"name":"color2","value":[1,0,0,1]&#125;] |
...
| `renderer.outline` | `object` | 是 | - | 默认轮廓线，包含线宽和颜色，注意：目前仅支持1px的线宽。对象结构示例：&#123;width: 1,color: [1, 1, 1, 1]&#125; |
...
| `renderer.classBreakInfos` | `array` | 否 | - | 可选，分类渲染器配置数组，对象结构如下： |
| `renderer.minValue` | `number` | 是 | - | 区间最小值 |
| `renderer.maxValue` | `number` | 是 | - | 区间最大值 |
| `renderer.symbol` | `object` | 是 | - | 不同分类的符号化配置对象，结构如下： |
| `renderer.color` | [`Color`](/docs/api/types#color) | 否 | - | 可选，二选一，区间值对应的颜色 |
| `renderer.size` | `number` | 否 | - | 可选，二选一，区间值对应的尺寸 |
| `renderer.height` | `number` | 否 | - | 可选，三选一，区间值对应的高度 |
| `renderer.visualVariables` | `array` | 是 | - | 简单渲染器支持根据字段属性控制颜色、尺寸和透明度的显示，参数如下： |
| `visibleRenderer` | `object` | 否 | - | 可选，要素可见性渲染器配置对象，包含的参数结构如下： |
| `visibleRenderer.rendererType` | [`RendererType`](/docs/api/types#renderertype) | 是 | - | (`RendererType`) 渲染器类型：可见性渲染器 |
| `visibleRenderer.field` | `string` | 是 | - | GeoJSON文件内的属性字段名称 |
| `visibleRenderer.fieldType` | [`FieldType`](/docs/api/types#fieldtype) | 是 | - | (`FieldType`) GeoJSON文件内的属性字段类型，枚举支持数值类型和字符串类型 |
| `visibleRenderer.defaultVisible` | `boolean` | 否 | true | 全局要素默认是否可见，默认值：true 可见 |
| `visibleRenderer.visibleValueInfos` | `array` | 否 | - | 可选，分类渲染器配置数组，对象结构如下： |
| `visibleRenderer.value` | `any` | 否 | - | 可选，field字段对应类型的值 |
| `visibleRenderer.minValue` | `number` | 否 | - | 可选，区间最小值，注意：数值类型字段可以使用此属性 |
| `visibleRenderer.maxValue` | `number` | 否 | - | 可选，区间最大值，注意：数值类型字段可以使用此属性 |
| `visibleRenderer.visible` | `boolean` | 是 | - | 必选，根据字段值或区间控制要素块是否显隐 |
| `materialRenderer` | `object` | 否 | - | 可选，自定义材质渲染器配置对象，包含的参数结构如下： |
| `materialRenderer.rendererType` | [`RendererType`](/docs/api/types#renderertype) | 是 | - | (`RendererType`) 渲染器类型：自定义材质渲染器 |
| `materialRenderer.field` | `string` | 是 | - | GeoJSON文件内的属性字段名称 |
| `materialRenderer.fieldType` | [`FieldType`](/docs/api/types#fieldtype) | 是 | - | (`FieldType`) GeoJSON文件内的属性字段类型，枚举支持数值类型和字符串类型 |
| `materialRenderer.defaultMaterial` | `boolean` | 否 | 0 | 全局默认使用的自定义材质索引，默认值：0 |
| `materialRenderer.materialValueInfos` | `array` | 否 | - | 可选，自定义材质渲染器配置数组，每一个对象结构如下： |
| `materialRenderer.index` | `number` | 是 | - | 必选，根据字段值或区间控制要素块是否显隐 |
| `materialRenderer.value` | `any` | 否 | - | 可选，field字段对应类型的值 |
| `materialRenderer.minValue` | `number` | 否 | - | 可选，区间最小值，注意：数值类型字段可以使用此属性 |
| `materialRenderer.maxValue` | `number` | 否 | - | 可选，区间最大值，注意：数值类型字段可以使用此属性 |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | 要删除的GeoJSONLayer对象的ID或者ID数组（可以删除一个或者多个） |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | GeoJSONLayer图层对象的ID或者ID数组 |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `id` | `string` | 是 | - | GeoJSONLayer图层对象的ID |
| `featureId` | `number` | 是 | - | 要素区域Feature的ID |
| `distance` | `number` | 否 | - | 可选参数，观察点距离目标点（被拍摄物体）的距离，取值范围：[0.01~任意正数]，如果设置为0或者不设置，系统自动计算 |
| `flyTime` | `number` | 否 | 2秒 | 可选参数，相机飞行的时间，取值范围：[0~任意正数]，单位：秒，默认值2秒 |
| `rotation` | `array` | 否 | - | 可选参数，相机旋转的欧拉角：[Pitch,Yaw,Roll]，数组元素类型：(number)，取值范围：Pitch[-90~90] Yaw[-180~180] Roll[0] |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | GeoJSONLayer对象的ID或者ID数组 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
|------|------|:----:|--------|------|
| `id` | `string` | 是 | - | GeoJSONLayer图层对象的ID |
| `featureId` | `number` | 是 | - | GeoJSONLayer图层对象内部的要素区域ID |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
|------|------|:----:|--------|------|
| `id` | `string` | 是 | - | GeoJSONLayer图层对象的ID |
| `fieldName` | `string` | 是 | - | GeoJSONLayer图层对象内部的要素包含的属性字段名称 |
| `fieldType` | `number` | 是 | - | GeoJSONLayer图层对象内部的要素包含的属性字段类型 |
| `values` | `array` | 是 | - | GeoJSONLayer图层对象内部的要素区域ID，数组元素类型为number |
...
|------|------|:----:|--------|------|
| `id` | `string` | 是 | - | GeoJSONLayer图层对象的ID |
| `featureIds` | `array` | 是 | - | GeoJSONLayer图层对象内部的要素区域ID，数组元素类型为number |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | GeoJSONLayer图层对象的ID或ID数组 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
```js
await fdapi.geoJSONLayer.unHighlightAllFeaturesById(['对象ID']);
```
...
```js
await fdapi.geoJSONLayer.unHighlightFeature('对象ID', 0);
```
...
```js
await fdapi.geoJSONLayer.unHighlightFeatures({
    id: '对象ID',
    featureIds: []
});
```
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `option` | `object` | 是 | - | 加载GeoJSON的配置项对象，结构参考add()方法 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
