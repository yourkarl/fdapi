...
![TileLayer 示意图 1](/img/refdoc/api/tilelayer-building.gif)

![TileLayer 示意图 2](/img/refdoc/api/tilelayer-highlight.gif)
...
- **适用行业**：智慧城市与实景三维、智慧园区、规划设计、智慧水利、能源、智慧交通与公路、轨道交通、智慧工地、智慧工厂、石油化工、智慧矿山、地下综合管线。
- **使用场景**：
  - 城市级实景三维底座的加载与浏览
  - 建筑/部件单体化与点选高亮
  - 专题样式着色与剖切分析
  - 公路 / 路桥数字孪生：倾斜摄影与道路、桥梁 BIM 统一加载，构件单体拾取、几何与业务属性查询（getActorInfoFromDB），面 / 体剖切与 X 光透视查看内部结构与预埋管线。
  - 轨道交通 BIM+GIS：线路、隧道、车站 BIM 单体化与属性查询，贯穿设计 / 施工 / 运维全生命期。
  - 厂区 / 车间 / 炼化装置 / 巷道 / 管廊 BIM 单体化拾取，配合 enableClip 剖切透视与 getActorInfoFromDB 关联台账。
- **注意事项**：
...
| `fileName` | `string` | 是 | - | 3dt文件路径，推荐使用@path 方式管理添加3dt图层资源 |
| `password` | `string` | 否 | - | 可选，3dt文件路径的私有秘钥 |
| `wkt` | `string` | 否 | - | 可选，球面坐标系下支持对倾斜摄影类型的3dt进行重投影，格式支持EPSG编码如："EPSG:4547"或常规的wkt字符串 |
| `visible` | `boolean` | 否 | true | 可选，添加后是否可见，默认值：true 可见 |
| `viewHeightRange` | `array` | 否 | [-1000000000, 1000000000] | 可选，可见高度范围：[最小可见高度, 最大可见高度]，默认值: [-1000000000, 1000000000] |
| `releaseWhenHidden` | `boolean` | 否 | false | 可选，隐藏图层时是否释放资源，默认值：false 不释放 |
| `location` | `array` | 是 | - | 位置坐标：[X,Y,Z]，[取值示例](/docs/tutorials/coordinates)，数组元素类型：(number)，取值范围：[任意数值]，注意：如果工程是球面坐标系则此参数相当于offset，仅表示图层位置的偏移量，非坐标值，单位：米 |
| `rotation` | `array` | 是 | - | 旋转：[Pitch,Yaw,Roll]，数组元素类型：(number)，取值范围：[任意数值] |
| `scale` | `array` | 是 | - | 缩放：[X,Y,Z]，数组元素类型：(number)，取值范围：[任意正整数] |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `data` | `object \| array` | 是 | - | 对象或数组类型，如果是数组类型，对于每个数组元素，有以下属性： |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 属性 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `id` | `string` | 是 | - | 挖洞操作唯一标识符 |
| `tileLayerId` | `string` | 是 | - | TileLayer对象的ID |
| `coordinates` | `array` | 是 | - | 挖洞多边形的坐标数组，二维数组，数组元素类型：挖洞多边形的坐标，[取值示例](/docs/tutorials/coordinates) |
| `innerRings` | `array` | 是 | - | 挖洞多边形的内环，三维数组，数组元素类型：挖洞多边形的坐标数组 |
| `isReverseCut` | `boolean` | 否 | false | 挖洞多边形是否反转，默认值：false |
...
|------|------|:----:|--------|------|
| `tileLayerId` | `string` | 是 | - | TileLayer对象的ID |
| `shapeFilePath` | `string` | 是 | - | shapeFilePath文件路径，支持本地路径和网络路径，[资源引入说明](/docs/tutorials/resources) |
...
| `id` | `string` | 是 | - | 唯一标识符 |
| `tileLayerId` | `string` | 是 | - | TileLayer对象的ID |
| `coordinates` | `array` | 是 | - | 压平多边形的坐标，[取值示例](/docs/tutorials/coordinates) |
| `ententBufferSize` | `number` | 是 | - | 羽化范围，取值范围：[任意正浮点数]，单位：米 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
```js
await fdapi.tileLayer.addModifier('对象ID', '对象ID', [0, 0, 0], 0);
```
...
| `id` | `string` | 是 | - | 压平操作唯一标识符 |
| `tileLayerId` | `string` | 是 | - | TileLayer对象的ID |
| `coordinates` | `array` | 是 | - | 压平多边形的坐标数组，二维数组，数组元素类型：压平多边形的坐标，[取值示例](/docs/tutorials/coordinates) |
| `innerRings` | `array` | 是 | - | 压平多边形的内环，三维数组，数组元素类型：压平多边形的坐标数组 |
| `ententBufferSize` | `number` | 是 | - | 羽化范围，取值范围：[任意正浮点数]，单位：米 |
...
```js
await fdapi.tileLayer.addModifiers({
    id: '对象ID',
    tileLayerId: '对象ID',
    coordinates: [0, 0, 0],
    innerRings: [],
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | TileLayer对象的ID或数组 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
```js
await fdapi.tileLayer.clearHole(['对象ID']);
```
...
```js
await fdapi.tileLayer.clearModifier(['对象ID']);
```
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | 要删除的TileLayer对象的ID或者ID数组（可以删除一个或者多个） |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
```js
await fdapi.tileLayer.deleteHole('对象ID', '对象ID');
```
...
```js
await fdapi.tileLayer.deleteModifier('对象ID', '对象ID');
```
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `number \| array` | 是 | - | TileLayer的ID或者ID数组 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
fdapi.tileLayer.disableXRay('对象ID');
```
...
| 属性 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `tileLayerId` | `string` | 是 | - | 图层Id |
| `enable` | `boolean` | 是 | - | 图层是否支持对贴花类型对象的贴合 |
...
|------|------|:----:|--------|------|
| `tileLayerId` | `string` | 是 | - | 图层Id |
| `supportFluid` | `boolean` | 否 | true | 图层是否支持水流体对象Fluid，默认值：true |
...
|------|------|:----:|--------|------|
| `tileLayerId` | `string` | 是 | - | 图层ID |
| `enable` | `boolean` | 是 | - | 网络图层服务是否贴合该3dt图层 |
...
```js
await fdapi.tileLayer.enableImageLayerDecal({
    tileLayerId: '对象ID',
    enable: true
});
```
...
![TileLayer · enableXRay(ids, color, fn) 示意](/img/refdoc/api/TileLayer.EnableXRay.png)
...
```js
fdapi.camera.set(492544.504063, 2492199.183398, 27.880498, -33.256496, -39.439247, 0);
fdapi.tileLayer.enableXRay('对象ID', [1, 1, 1, 0.0381]);
```
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | TileLayer对象的ID或者ID数组 |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `id` | `string` | 是 | - | TileLayer的ID |
| `objectId` | `string` | 是 | - | Actor的ID |
| `distance` | `number` | 否 | - | 可选参数，观察点距离目标点（被拍摄物体）的距离，取值范围：[0.01~任意正数]，如果设置为0或者不设置，系统自动计算 |
| `flyTime` | `number` | 否 | 2秒 | 可选参数，相机飞行的时间，取值范围：[0~任意正数]，单位：秒，默认值2秒 |
| `rotation` | `array` | 否 | - | 可选参数，相机旋转的欧拉角：[Pitch,Yaw,Roll]，数组元素类型：(number)，取值范围：Pitch[-90~90] Yaw[-180~180] Roll[0] |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
```js
await fdapi.tileLayer.focusActor('对象ID', '对象ID', 0, '2秒', [0, 0, 0]);
```
...
|------|------|:----:|--------|------|
| `id` | `string` | 是 | - | TileLayer的ID |
| `objectIds` | `array` | 是 | - | TileLayer里的Actor的ObjectID（单个或者数组） |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | 要获取的TileLayer对象ID或者ID数组（可以获取一个或者多个） |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 属性 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `id` | `string` | 是 | - | TileLayer对象的ID |
| `objectIds` | `array` | 是 | - | TileLayer里的Actor的ObjectID字符串（单个或者数组） |
...
![TileLayer · getActorInfoFromDB(data, fn) 示意](/img/refdoc/api/TileLayer.getInformation.png)

| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `data` | `array \| object` | 是 | - | 对象或数组类型，如果是数组类型，每个元素有以下属性：tileLayerId, objectIds |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `tileLayerIds` | `string \| array` | 是 | - | TileLayer对象的ID或ID数组 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
|------|------|:----:|--------|------|
| `id` | `string` | 是 | - | 点云图层的Id |
| `name` | `string` | 是 | - | 点云图层包含的字段名称 |
| `values` | `array` | 是 | - | 点云图层包含的字段名称对应的取值数组，支持多个匹配到的值 |
| `color` | `number` | 是 | - | 匹配到的box盒子待设置的颜色 |
...
```js
await fdapi.tileLayer.getDBTabID({
    id: '对象ID',
    name: '示例值',
    values: [],
    color: 0
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | TileLayer的ID或者ID数组 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
```js
//查询小别墅包含的构件id
fdapi.tileLayer.getObjectIDs('对象ID');
```
...
```js
await fdapi.tileLayer.hideActor('对象ID', '对象ID');
```
...
```js
await fdapi.tileLayer.hideActors({
    id: '对象ID',
    objectIds: []
});
```
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `tileLayerIds` | `string \| array` | 是 | - | TileLayer的ID或ID数组 |
...
fdapi.tileLayer.hideAllActors('对象ID');
```
...
![TileLayer · highlightActor(id, objectId, fn) 示意](/img/refdoc/api/tilelayer.HighlightActor.gif)
...
```js
await fdapi.tileLayer.highlightActor('对象ID', '对象ID');
```
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `data` | `object \| array` | 是 | - | 数据对象，支持以下属性： |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
```js
await fdapi.tileLayer.highlightActors({
    id: '对象ID',
    objectIds: []
});
```
...
| `id` | `string` | 是 | - | TileLayer的ID |
| `objectIds` | `array` | 是 | - | TileLayer里的Actor的ObjectID（单个或者数组） |
| `color` | [`Color`](/docs/api/types#color) | 否 | Color.Blue | Actor的高亮颜色，默认值:Color.Blue，支持四种格式，[取值示例](/docs/tutorials/color) |
| `bWireframe` | `boolean` | 否 | true | 高亮时是否使用线框模式，默认值：true，设置为false则是填充模式 |
...
```js
await fdapi.tileLayer.highlightActorsWithColor({
    id: '对象ID',
    objectIds: [],
    bWireframe: true
});
...
```js
await fdapi.tileLayer.highlightActorWithColor('对象ID', '对象ID', Color.Blue, true);
```
...
| `id` | `string` | 是 | - | 点云图层的Id |
| `name` | `string` | 是 | - | 点云图层包含的字段名称 |
| `values` | `array` | 是 | - | 点云图层包含的字段名称对应的取值数组，支持高亮多个匹配到的值 |
| `glow` | `boolean` | 否 | true | 高亮时是否闪烁，默认值：true |
| `bCustomColor` | `boolean` | 否 | false | 是否开启自定义颜色，默认值：false |
| `color` | [`Color`](/docs/api/types#color) | 否 | Color.White | 自定义颜色值，仅当bCustomColor开启后自定义颜色生效，默认值:Color.White，支持四种格式，[取值示例](/docs/tutorials/color) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `queryOption` | `QueryOption` | 是 | - | 查询条件对象 请参考： [`QueryOption`](/docs/api/analysis/query-option) |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
```js
await fdapi.tileLayer.query('示例值');
```
...
|------|------|:----:|--------|------|
| `tileLayerIds` | `string \| array` | 是 | - | TileLayer的ID或ID数组 |
| `colors` | `array` | 是 | - | 颜色分层调色板数组，数组包含的每一个对象包含高度属性和对应颜色值 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 属性 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `value` | `number` | 是 | - | 3dt图层的高度 |
| `color` | `array` | 是 | - | 高度对应的颜色值，示例：[1,0,0,1] |
...
```js
await fdapi.tileLayer.setAltitudeHeatMap([255, 255, 255]);
```
...
|------|------|:----:|--------|------|
| `tileLayerIds` | `string \| array` | 是 | - | TileLayer对象的ID或ID数组 |
| `enabled` | `boolean` | 是 | - | 是否开启碰撞检测的总开关，如果此参数设置为false，则下面三个参数均会失效 |
| `mouseInteract` | `boolean` | 否 | true | 是否开启鼠标交互，默认值：true 开启 |
| `mouseFunction` | `boolean` | 否 | true | 是否开启鼠标相关的功能交互，包含鼠标拾取、分析工具、测量工具等，默认值：true 开启 |
| `characterCollision` | `boolean` | 否 | true | 是否开启角色碰撞，默认值：true 开启角色碰撞 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
```js
await fdapi.tileLayer.setCollision(true, true, true, true);
```
...
```js
await fdapi.tileLayer.setFileName('对象ID', '示例值');
```
...
|------|------|:----:|--------|------|
| `id` | `string` | 是 | - | TileLayer的ID |
| `newVal` | `array` | 是 | - | 新的位置坐标：[X,Y,Z]，[取值示例](/docs/tutorials/coordinates)，数组元素类型：(number)，取值范围：[任意数值] |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
```js
await fdapi.tileLayer.setLocation('对象ID', []);
```
...
| `name` | `string` | 是 | - | 点云图层包含的字段名称 |
| `values` | `array` | 是 | - | 点云图层包含的字段名称对应的取值数组，支持多个匹配到的值 |
| `visible` | `number` | 是 | - | 点云字段匹配到的值的box的可见性，0不可见 1可见 |
...
```js
await fdapi.tileLayer.setPointCloudBoxVisible({
    id: '对象ID',
    name: '示例值',
    values: [],
    visible: 0
...
|------|------|:----:|--------|------|
| `id` | `string` | 是 | - | TileLayer的ID |
| `pointCloudSize` | `number` | 是 | - | 点云缩放的倍数，取值范围：[0.01~4] |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
|------|------|:----:|--------|------|
| `id` | `string` | 是 | - | 点云图层的Id |
| `collision` | `boolean` | 是 | - | 是否开启碰撞 |
| `renderer` | `object` | 是 | - | 必选，渲染器配置对象参数如下： |
| `renderer.rendererType` | [`RendererType`](/docs/api/types#renderertype) | 是 | - | (`RendererType`) 渲染器类型，包含三种：简单渲染器、唯一值渲染器、分类渲染器 |
| `renderer.materialType` | `number` | 是 | - | 渲染材质类型，取值：[0,1,2]，分别对应三种：X光、Mask不透明、半透明 |
| `renderer.type` | [`VisualType`](/docs/api/types#visualtype) | 是 | - | (`VisualType`) 控制显示的可视化类型枚举 |
| `renderer.renderBox` | `number` | 否 | 0 | 是否开启盒子模式渲染，取值：[0,1]，默认值：0 |
| `renderer.boxSize` | `array` | 是 | - | 盒子模式下体块的尺寸，即长宽高[X,Y,Z]，单位：米 |
| `renderer.renderBoxWireframe` | `number` | 是 | - | 盒子模式下是否显示线框，取值：[0,1,2]，0不显示 1显示 2仅显示线框（隐藏面） |
| `renderer.wireframeColor` | `color` | 是 | - | 盒子模式下线框的颜色 |
| `renderer.wireThickness` | `number` | 是 | - | 盒子模式下线框的尺寸 |
| `renderer.lightFactor` | `number` | 是 | - | 亮度因子 |
| `renderer.intensity` | `number` | 是 | - | 亮度值 |
| `renderer.frequency` | `number` | 是 | - | 频次 |
| `renderer.splitFactor` | `number` | 是 | - | 点云裂分因子，值越大绘制精度越低，值越小绘制精度则越高（会更消耗显卡资源），取值范围：[0,任意正数] |
| `renderer.autoScaleDepth` | `number` | 是 | - | 盒子模式下，盒子尺寸自动缩放的相机临界深度，取值范围：[0,任意正数] |
| `renderer.field` | `string` | 是 | - | 点云图层包含的属性字段名称 |
| `renderer.fieldType` | [`FieldType`](/docs/api/types#fieldtype) | 是 | - | (`FieldType`) 点云图层包含的属性字段类型，枚举支持数值类型和字符串类型 |
| `renderer.gradient` | `boolean` | 是 | - | 颜色是否差值渐变 |
| `renderer.defaultSymbol` | `object` | 是 | - | 必选，默认符号化配置参数如下： |
| `renderer.symbolType` | `number` | 是 | - | 符号化类型，0 simple-marker圆形点填充 1 simple-line线填充(注意：目前仅支持1px的线宽) 2 simple-fill面填充 3 polygon3d填充 |
| `renderer.size` | `number` | 否 | - | 可选，点的默认尺寸，仅针对simple-marker圆形填充生效； |
| `renderer.height` | `number` | 否 | - | 可选，polygon3d的默认高度，仅针对polygon3d填充生效； |
| `renderer.color` | `array` | 是 | - | 默认填充颜色 |
| `renderer.outline` | `-` | 是 | - | — |
| `renderer.uniqueValueInfos` | `array` | 否 | - | 可选，唯一值渲染器配置数组，对象结构如下： |
| `renderer.value` | `any` | 是 | - | field字段对应值 |
| `renderer.symbol` | `object` | 是 | - | 各唯一值对应的符号化配置对象，结构如下： |
| `renderer.color` | [`Color`](/docs/api/types#color) | 否 | - | 可选，三选一，唯一值对应的颜色 |
| `renderer.size` | `number` | 否 | - | 可选，三选一，唯一值对应的尺寸 |
| `renderer.height` | `number` | 否 | - | 可选，三选一，唯一值对应的高度 |
...
|------|------|:----:|--------|------|
| `id` | `string` | 是 | - | TileLayer的ID |
| `newVal` | `array` | 是 | - | 新的旋转坐标：[Pitch,Yaw,Roll]，数组元素类型：(number)，取值范围：[任意数值] |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
```js
await fdapi.tileLayer.setRotation('对象ID', []);
```
...
|------|------|:----:|--------|------|
| `id` | `string` | 是 | - | TileLayer的ID |
| `newVal` | `any` | 是 | - | 新的缩放：[X,Y,Z]，数组元素类型：(number)，取值范围：[任意正整数] |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
```js
await fdapi.tileLayer.setScale('对象ID', '示例值');
```
...
![TileLayer · setStyle(tileLayerIds, style, color, saturation, brightness, contrast, contrastBase, fn) 示意](/img/refdoc/api/TileLayer.SetStyle.gif)

| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `tileLayerIds` | `string \| array` | 是 | - | TileLayer的ID或ID数组 |
| `style` | `number` | 是 | - | 样式， 0：默认；1：X光；2：纯色；3：水晶体；4：暗黑；5：科幻；6：扩散 |
| `color` | [`Color`](/docs/api/types#color) | 否 | Color.White | 颜色，默认值:Color.White，支持四种格式，[取值示例](/docs/tutorials/color) |
| `saturation` | `number` | 否 | 1.0 | 饱和度，仅在默认样式0下生效，取值范围：[0~2]，默认值：1.0 |
| `brightness` | `number` | 否 | 1.0 | 亮度 ，仅在默认样式0下生效，取值范围：[0.1~10]，默认值：1.0 |
| `contrast` | `number` | 否 | 1.0 | 对比度，仅在默认样式0下生效，取值范围：[0.2~5]，默认值：1.0 |
| `contrastBase` | `number` | 否 | 0.18 | 对比度基准，仅在默认样式0下生效，取值范围：[0.036~0.9]，默认值：0.18 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
```js
await fdapi.tileLayer.setStyle(0, Color.White, 1.0, 1.0, 1.0, 0.18);
```
...
//设置小别墅的可见高度范围
fdapi.tileLayer.setViewHeightRange('对象ID', 1, 100);
```
...
//小别墅 仅视口1和视口3可见
fdapi.tileLayer.setViewportVisible('对象ID', Viewport.V1 | Viewport.V3);
```
...
```js
await fdapi.tileLayer.showActor('对象ID', '对象ID');
```
...
![TileLayer · showActors(data, fn) 示意](/img/refdoc/api/tilelayer.HideActor.gif)
...
```js
await fdapi.tileLayer.showActors({
    id: '对象ID',
    objectIds: []
});
```
...
//相机移动到对应actors范围内
fdapi.camera.set(492533.451055, 2492188.38875, 30.26697, -34.987709, -47.59425, 0);
fdapi.tileLayer.showAllActors('对象ID');
```
...
```js
await fdapi.tileLayer.unHighlightActor('对象ID', '对象ID');
```
...
```js
await fdapi.tileLayer.unHighlightActors({
    id: '对象ID',
    objectIds: []
});
```
...
| `id` | `string` | 是 | - | 点云图层的Id |
| `name` | `string` | 是 | - | 点云图层包含的字段名称 |
| `values` | `array` | 是 | - | 点云图层包含的字段名称对应的取值数组，支持取消高亮多个匹配到的值 |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `data` | `object \| array` | 是 | - | 数据结构，支持更新以下属性： |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 属性 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `id` | `string` | 是 | - | 根据TileLayer对象的ID更新以下属性 |
| `groupId` | `string` | 否 | - | 可选，Group分组 |
| `userData` | `string` | 否 | - | 可选，用户自定义数据 |
| `fileName` | `string` | 是 | - | 3dt文件路径，推荐使用@path 方式管理添加3dt图层资源 |
...
| `id` | `string` | 是 | - | 唯一标识符 |
| `tileLayerId` | `string` | 是 | - | TileLayer对象的ID |
| `holeCoordinate` | `array` | 是 | - | 待修改挖洞的坐标，[取值示例](/docs/tutorials/coordinates) |
| `isReverseCut` | `boolean` | 否 | false | 挖洞多边形是否反转，默认值：false |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
```js
await fdapi.tileLayer.updateHole('对象ID', '对象ID', [0, 0, 0], false);
```
...
```js
await fdapi.tileLayer.updateModifier('对象ID', '对象ID', [0, 0, 0], 0);
```
...
|------|------|:----:|--------|------|
| `id` | `string` | 是 | - | tileLayer的ID |
| `newValMap` | `object` | 是 | - | 要设置的新值格式：&#123;Key:Value&#125; |
| `where` | `string` | 是 | - | 查询条件 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
```js
await fdapi.tileLayer.updateRecord('对象ID', {}, '示例值');
```
...
```js
fdapi.camera.set(492544.504063, 2492199.183398, 27.880498, -33.256496, -39.439247, 0);
fdapi.tileLayer.hideActor('对象ID', "a6aa132d-ccd7-408f-b2f9-ed67350c8c3a-0003b64a");
```
...
```js
fdapi.camera.set(492544.504063, 2492199.183398, 27.880498, -33.256496, -39.439247, 0);
fdapi.tileLayer.showActor('对象ID', "a6aa132d-ccd7-408f-b2f9-ed67350c8c3a-0003b64a");
```
...
```js
fdapi.camera.set(492544.504063, 2492199.183398, 27.880498, -33.256496, -39.439247, 0);
fdapi.tileLayer.focusActor('对象ID', "a6aa132d-ccd7-408f-b2f9-ed67350c8c3a-0003b64a", 10, 1, [-45, 45, 0]);
```
...
//高亮小别墅房顶构件
fdapi.tileLayer.highlightActor('对象ID', "a6aa132d-ccd7-408f-b2f9-ed67350c8c3a-0003b64a");
```
...
```js
fdapi.camera.set(492544.504063, 2492199.183398, 27.880498, -33.256496, -39.439247, 0);
fdapi.tileLayer.highlightActorWithColor('对象ID', ["a6aa132d-ccd7-408f-b2f9-ed67350c8c3a-0003b64a", "98e2f96c-ccbd-4abb-a697-e7e5136106ee-000cf488"], [0, 1, 0, 0.1], false);
```
...
```js
fdapi.camera.set(492544.504063, 2492199.183398, 27.880498, -33.256496, -39.439247, 0);
fdapi.tileLayer.unHighlightActor('对象ID', "a6aa132d-ccd7-408f-b2f9-ed67350c8c3a-0003b64a");
```
...
let result = await fdapi.tileLayer.getObjectIDs('对象ID');
