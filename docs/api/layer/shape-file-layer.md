...
- **适用行业**：GIS 与规划、智慧城市、智慧水利、国土、测绘、智慧交通与公路、测绘地理信息、林业。
- **使用场景**：
  - 行政区划、地块、路网等矢量底图加载
  - 专题要素按属性着色与分级
  - 矢量数据的空间与属性查询
  - 一张图矢量边界：行政区划、园区边界 SHP 加载与拾取高亮。
- **注意事项**：
  - 林班 / 地类图斑 / 界址矢量等测绘专题面加载，配合 groupId 分专题显隐与 highlightArea 高亮。
...
| `file` | `String` | 是 | - | ShapeFileLayer的文件路径，支持本地路径和网络路径 |
| `offset` | `array` | 否 | [0.000000, 0.000000, 0.000000] | 偏移量：[X,Y,Z]，数组元素类型：(number)，默认值[0.000000, 0.000000, 0.000000] |
| `rotation` | `array` | 否 | [0.000000, 0.000000, 0.000000] | 旋转：[Pitch,Yaw,Roll]，数组元素类型：(number)，取值范围：[任意数值]，默认值[0.000000, 0.000000, 0.000000] |
| `defaultColor` | [`Color`](/docs/api/types#color) | 是 | - | 支持四种格式，[取值示例](/docs/tutorials/color) 默认颜色：[0.000000, 0.000000, 1.000000, 1.000000] RGBA |
| `polygonColorFieldName` | `String` | 否 | - | 当type==Polygon时此属性可选，多边形颜色字段 |
| `polygonHeightFieldName` | `String` | 否 | - | 当type==Polygon时此属性可选，多边形高度字段 |
| `polygonGroundHeightFieldName` | `String` | 否 | - | 当type==Polygon时此属性可选，底面高度字段 |
| `polygonTopHeightFieldName` | `String` | 否 | - | 当type==Polygon时此属性可选，顶面高度字段 |
| `polygonStyle` | `number` | 否 | - | 当type==Polygon时此属性可选，多边形样式，共6种，取值范围：[0~5]，分别为 0：体块（半透明） 1：圆点 2：单色 3：体块（纯色）4：纯色（半透明） 5：X光 |
| `polygonDefaultHeight` | `number` | 否 | - | 当type==Polygon时此属性可选，多边形高度，单位米 |
| `cacheAllField` | `boolean` | 否 | false | 当type==Polygon时此属性可选，Polygon支持缓存所有字段，以便于高效动态更新Polygon不同字段的效果，默认值：false，注意：仅在只更新Polygon字段时生效 |
| `polylineDefaultThickness` | `number` | 否 | - | 当type==Polyline时此属性可选，折线宽度，单位米 |
| `pointTextContentFieldName` | `String` | 否 | - | 当type==Point时此属性可选，文本内容（点字段） |
| `pointDisplayMode` | `number` | 否 | - | 当type==Point时此属性可选，显示模式，取值说明如下： |
| `clusterByImage` | `boolean` | 否 | - | 当type==Point时此属性可选，是否按相同图片路径(pointImage)聚合显示 |
| `pointImage` | `String` | 否 | - | 当type==Point时此属性可选，图片路径 |
| `pointContentRange` | `number` | 否 | - | 当type==Point时此属性可选，文本可见距离，单位米 |
| `pointRange` | `array` | 否 | - | 当type==Point时此属性可选，可见距离（米）点坐标：[X,Y] |
| `pointFontColor` | [`Color`](/docs/api/types#color) | 否 | - | 当type==Point时此属性可选，字体颜色 ，支持四种格式，[取值示例](/docs/tutorials/color) |
| `pointFontOutlineColor` | [`Color`](/docs/api/types#color) | 否 | - | 当type==Point时此属性可选，字体轮廓颜色 ，支持四种格式，[取值示例](/docs/tutorials/color) |
| `pointBackgroundColor` | [`Color`](/docs/api/types#color) | 否 | - | 当type==Point时此属性可选，字体背景颜色 ，支持四种格式，[取值示例](/docs/tutorials/color) |
| `pointShowLine` | `boolean` | 否 | - | 当type==Point时此属性可选，是否显示线 默认false |
| `pointOcclusionCull` | `boolean` | 否 | - | 当type==Point时此属性可选，是否参与遮挡 默认true |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | 要删除的ShapeFileLayer对象的ID或者ID数组（可以删除一个或者多个） |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | ShapeFileLayer对象的ID或者ID数组 |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `shapeFileLayerId` | `string` | 是 | - | ShapeFileLayer对象的ID |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | 要获取的ShapeFileLayer对象ID或者ID数组（可以获取一个或者多个） |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 属性 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `shpId` | `string` | 是 | - | ShapeFileLayer对象的ID |
| `featureIds` | `array` | 是 | - | ShapeFileLayer对象内部的要素区域ID，数组元素类型为number |
...
|------|------|:----:|--------|------|
| `shapeFileLayerId` | `string` | 是 | - | ShapeFileLayer对象的ID |
| `featureId` | `number` | 是 | - | ShapeFileLayer对象内部的要素区域ID |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 属性 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `file` | `string` | 是 | - | ShapeFileLayer文件的路径 |
...
```js
await fdapi.shapeFileLayer.unHighlightFeature('对象ID', 0);
```
...
```js
await fdapi.shapeFileLayer.unHighlightFeatures({
    shapeFileLayerId: '对象ID',
    featureIds: []
});
```
...
| 属性 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `id` | `string` | 是 | - | 根据ShapeFileLayer对象的ID更新以下属性 |
| `groupId` | `string` | 否 | - | 可选，Group分组 |
| `userData` | `string` | 否 | - | 可选，用户自定义数据 |
| `offset` | `array` | 否 | [0.000000, 0.000000, 0.000000] | 偏移量：[X,Y,Z]，数组元素类型：(number)，默认值[0.000000, 0.000000, 0.000000] |
| `rotation` | `array` | 否 | [0.000000, 0.000000, 0.000000] | 旋转：[Pitch,Yaw,Roll]，数组元素类型：(number)，取值范围：[任意数值]，默认值[0.000000, 0.000000, 0.000000] |
