...
- **使用场景**：
  ...
  - 通用提示与辅助交互
  - 零散功能的调用
  - 汇报与协同：双视口方案比选（左右分屏，相机 / 缩放 / 剖切双向同步）、远程同步鼠标（手型光标跟随）、汇报模式导出 4K/8K 高清素材。
- **注意事项**：
  - 功能较杂，按需取用
  - UI 类接口注意与页面布局协调
  - 不同版本可能存在差异
...
|------|------|:----:|--------|------|
| `id` | `number` | 是 | - | 按钮的ID |
| `x` | `number` | 是 | - | 图片按钮的位置:x坐标 |
| `y` | `number` | 是 | - | 图片按钮的位置:y坐标 |
| `width` | `number` | 是 | - | 图片按钮的宽度，单位像素 |
| `height` | `number` | 是 | - | 图片按钮的高度，单位像素 |
| `imageSequecePath` | `string` | 是 | - | 序列贴图的路径，[资源引入说明](/docs/tutorials/resources) |
| `imageSequeceLength` | `string` | 是 | - | 序列贴图的图片数量，也就是帧数 |
| `loop` | `boolean` | 是 | - | 是否循环播放序列贴图 |
| `interactable` | `boolean` | 是 | - | 是否可以用鼠标点击操作 |
...
```js
await fdapi.misc.addAnimatedImageButtons({
    id: 0,
    x: 0,
    y: 0,
    width: 0,
    height: 0,
    imageSequecePath: HostConfig.Path + '/assets/...',
    imageSequeceLength: '示例值',
    loop: true,
    interactable: true
});
```
...
|------|------|:----:|--------|------|
| `id` | `string` | 是 | - | 按钮的ID |
| `x` | `number` | 是 | - | Button的屏幕坐标X |
| `y` | `number` | 是 | - | Button的屏幕坐标Y |
| `width` | `number` | 是 | - | Button的宽度 |
| `height` | `number` | 是 | - | Button的高度 |
| `normalImage` | `string` | 是 | - | Button正常显示的图片 |
| `hoverImage` | `string` | 是 | - | 鼠标移上去后显示的图片 |
| `tooltip` | `string` | 是 | - | Button的文字提示 |
...
```js
await fdapi.misc.addImageButtons({
    id: '对象ID',
    x: 0,
    y: 0,
    width: 0,
...
| 属性 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `actorTag` | `string` | 是 | - | 发布蓝图函数时在模型包含的Actor上添加的tag，调用前需和设计蓝图函数的开发人员确认 |
| `objectName` | `string` | 是 | - | 模型包含的Actor对象的ID，可以根据__g.tileLayer.getObjectIDs(tileLayerIds)方法获取 |
| `functionName` | `string` | 是 | - | 待调用的蓝图函数名称，调用前需和设计蓝图函数的开发人员确认此函数已存在 |
| `paramType` | [`BPFuncParamType`](/docs/api/types#bpfuncparamtype) | 是 | - | 传入单个参数，待传入参数类型 参考`BPFuncParamType`，如果需传递对象类型参数可以把Json对象定义为StringArray类型，在蓝图函数内部自己实现反序列化解析。 |
| `paramValue` | `any` | 是 | - | 传入单个参数，根据参数类型设置对应参数值 |
| `parameters` | `array` | 是 | - | 传入多个参数，数组类型，注意：传入多参数的顺序与类型务必与蓝图函数的参数顺序及其参数类型一致以保证执行结果符合预期。多个参数结构示例：[&#123;"paramType":BPFuncParamType.String,"paramValue":"示例值"&#125;,&#123;"paramType":BPFuncParamType.Bool,"paramValue":false&#125;,&#123;"paramType":BPFuncParamType.Float,"paramValue":100.8&#125;] |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `array` | 是 | - | 按钮的ID或数组（可以一次删除一个或者多个） |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
```js
await fdapi.misc.deleteImageButtons(['对象ID']);
```
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `viewportMode` | `number` | 是 | - | 视口布局类型，取值范围：[1~7]，参考上图从上到下从左到右计数，即录制导览动画时的视口布局 |
| `lineColor` | [`Color`](/docs/api/types#color) | 否 | #DEA309 | 可选参数，激活后视口边框线的颜色，默认值：#DEA309，支持四种格式，[取值示例](/docs/tutorials/color) |
| `lineSize` | `number` | 否 | 2px | 可选参数，激活后视口边框线的宽度，单位：像素，默认值：2px |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `assetPath` | `string \| array` | 是 | - | 模型Actor的路径或路径数组，可以从资源库的模型列表复制获取 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `pointArray` | `boolean` | 是 | - | 离散点坐标数组，数组元素格式：[X,Y] 注意：没有高度Z |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `materialPath` | `string \| array` | 是 | - | 单个材质路径或路径数组，可以从资源库的材质列表复制获取 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `url` | `string` | 是 | - | 影片文件路径，支持本地绝对路径和流媒体网络路径，注意：不支持工程本地的相对路径 |
| `loop` | `boolean` | 是 | - | 是否循环播放（from V2021.03.09） |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| `x` | `number` | 是 | - | 视频窗口的位置X |
| `y` | `number` | 是 | - | 视频窗口的位置Y |
| `width` | `number` | 是 | - | 视频窗口的宽度 |
| `height` | `number` | 是 | - | 视频窗口的高度 |
| `url` | `string` | 是 | - | 视频文件路径，支持本地绝对路径和流媒体网络路径，注意：不支持工程本地的相对路径 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `url` | `string` | 是 | - | 视频地址，支持网络地址或者本地文件。 |
| `options` | `object` | 是 | - | 视频播放窗口配置参数，支持以下参数： |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
|------|------|:----:|--------|------|
| `mute` | `boolean` | 是 | - | 是否静音。 |
| `x` | `number` | 是 | - | 视频播放窗口左上角距离显示器屏幕左侧的坐标 |
| `y` | `number` | 是 | - | 视频播放窗口左上角距离显示器屏幕顶部的坐标 |
| `cx` | `number` | 否 | 400 | 视频播放窗口宽度，默认值400 |
| `cy` | `number` | 否 | 300 | 视频播放窗口高度，默认值300 |
| `title` | `string` | 是 | - | 视频播放窗口标题栏文本，如果不设置，标题栏默认显示视频地址 |
| `opacity` | `number` | 是 | - | 视频播放窗口的透明度，取值范围：(0, 1] |
| `style` | `enum` | 否 | 2 | 视频播放窗口样式。 取值0：无标题栏，1：有标题栏，2：有标题栏，可拖动边框改变大小。 默认值为2。 |
| `hideBuffering` | `boolean` | 是 | - | 是否隐藏标题栏显示的视频流缓冲进度，默认不隐藏 |
| `maximizeBox` | `boolean` | 是 | - | 是否在标题栏显示最大化按钮，默认不显示 |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `type` | `number` | 是 | - | ACP工程包含的资产类型，枚举类型详情请参考 `AssetType` |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
```js
await fdapi.misc.projectAssetCount(0);
```
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `index` | `number \| array` | 是 | - | 被激活视口的索引或索引数组（同时激活多个视口），索引取值范围：[0~3] |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `fnCallback` | `function` | 是 | - | 回调函数，当收到服务器返回的版本信息后调用 |
...
```js
await fdapi.misc.setApiVersionReceived((version) => { console.log('SDK 版本:', version); });
```
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `isSync` | `boolean` | 是 | - | 相机是否同步，默认不联动：false |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `align` | `number` | 否 | 0 | 位置取值：0【底部】，1【居左】，2【居右】，默认0 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `playMode` | `number` | 否 | 0 | 播放模式取值： 0【单体播放】，1【单体循环】，2【整体循环】，默认0 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `isLinkage` | `boolean` | 是 | - | 相机是否联动 取值：联动true，不联动false，默认不联动false |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `id` | `number` | 是 | - | 成功调用playVideoAlone后返回的processId |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
```js
fdapi.misc.stopPlayVideoAlone(0);
```
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `onOff` | `boolean` | 是 | - | 打开关闭快捷键：true打开，false关闭 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
```js
await fdapi.misc.switchShortcutKey(true);
```
