...
![Camera 示意图 1](/img/refdoc/api/camera.gif)

![Camera 示意图 2](/img/refdoc/api/camera-1.gif)
...
- **使用场景**：
  ...
  - 一张图视角：基础地图缩放 / 平移浏览、重点对象定位与原始视点复位。
- **注意事项**：
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `path` | `string` | 是 | - | 导出正交投影图片的磁盘保存路径，注意：目前出图支持png和jpg格式，此路径需要在渲染服务器端存在，因为导出图片文件保存在渲染服务器的磁盘上 |
| `imageSize` | `array` | 是 | - | 导出正交投影图片的宽高分辨率：[width,height]，取值示例：[1920,1080] |
| `orthoWidth` | `number` | 是 | - | 导出正交投影图片的景深长度，单位：米 |
| `location` | `array` | 是 | - | 相机位置：[X,Y,Z]，[取值示例](/docs/tutorials/coordinates)，数组元素类型：(number)，取值范围：[任意数值] |
| `rotation` | `array` | 否 | [0,90,0] | 相机朝向欧拉角：[Pitch,Yaw,Roll]，数组元素类型：(number)，取值范围：Pitch[-90~90] Yaw[-180~180] Roll[0]，默认值：[0,90,0] |
| `backGroundColor` | [`Color`](/docs/api/types#color) | 否 | 黑色[0 | 导出正交投影图片的背景颜色，默认值：黑色[0, 0, 0, 1]，支持四种格式，[取值示例](/docs/tutorials/color) |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `location` | `array` | 是 | - | 必选，相机环绕位置：[X,Y,Z]，[取值示例](/docs/tutorials/coordinates)，数组元素类型：(number)，取值范围：[任意数值] |
| `rotation` | `array` | 否 | [0,90,0] | 可选，相机旋转的欧拉角：[Pitch,Yaw,Roll]，数组元素类型：(number)，取值范围：Pitch[-90~90] Yaw[-180~180] Roll[0]，默认值：[0,90,0] |
| `distance` | `number` | 否 | [1000] | 可选，相机观察距离，单位：米，默认值：[1000] |
| `time` | `number` | 否 | 10秒 | 可选，相机环绕一周的时间，取值范围：[0~任意正数]，单位：秒，默认值10秒 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `name` | `string` | 是 | - | 导览名称，可以根据getAnimationList()方法获取 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `startPoint` | `` | 是 | - | 起始点坐标位置，[取值示例](/docs/tutorials/coordinates) |
| `endPoint` | `` | 是 | - | 结束点坐标位置，[取值示例](/docs/tutorials/coordinates) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `bbox` | `` | 是 | - | 即3D Bounding Box，3D物体的包围盒，用于表示三维物体坐标的取值边界，格式示例：[minX,minY,minZ,maxX,maxY,maxZ] |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
```js
await fdapi.camera.lockByBBox('示例值');
```
...
| `pitch` | `number` | 否 | - | 上下旋转角度（俯仰），单位是度。可选参数，如果没有设置或者设置为0，系统会自动设置默认值。 |
| `yaw` | `number` | 否 | - | 左右旋转角度（航向），单位是度。可选参数，如果没有设置或者设置为0，系统会自动设置默认值。 |
...
fdapi.camera.lookAt(492035.37, 2488806.75, 402.62, 500, -15.0, -173.0, 0.2);
```
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `bbox` | `array` | 是 | - | bounding box |
| `pitch` | `number` | 否 | - | 上下旋转角度（俯仰），单位是度。 此参数可选，如果没有设置或者设置为0，系统会自动设置默认值。 |
| `yaw` | `number` | 否 | - | 左右旋转角度（航向），单位是度。 此参数可选，如果没有设置或者设置为0，系统会自动设置默认值。 |
| `flyTime` | `number` | 否 | 2秒 | 可选参数，相机飞行的时间，取值范围：[0~任意正数]，单位：秒，默认值2秒 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
![Camera · playAnimation(ids, mask, fn) 示意](/img/refdoc/api/PlayAnimation.gif)

| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `number \| array` | 是 | - | 待播放的动画导览的索引编号或索引编号数组 |
| `mask` | `number` | 否 | - | 可选，播放导览时传入对应掩码则使用录制导览时的工程对应设置，包含四类掩码：相机位置(Camera:0x1) 环境天气(Environment:0x2) 工程树属性(ProjectTree:0x4) 导览设置(Settings:0x8)，请参考 `AnimationMask` |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
![Camera · set(x, y, z, pitch, yaw, flyTime, fn) 示意](/img/refdoc/api/Camera.Set.gif)
...
```js
await fdapi.camera.useOldDataFormat(true);
```
