...
- **功能介绍**：Fluid 在指定包围盒（bbox）内进行实时流体仿真，通过出水点（sources）的位置、流速流向、形状与持续时间驱动水体流动，内置 28 种水样式，用于表达喷涌、漫流、注水等动态流体效果。
...
- **注意事项**：
  ...
  - 出水点 velocity（流速流向）取值范围 [-2,2]，属于效果型仿真而非严格数值水动力；需要真实水深/流场数据驱动时应使用 HydroDynamic2D 等对象。
