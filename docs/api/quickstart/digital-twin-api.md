...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `host` | `string` | 是 | - | 服务器地址，格式如下 IP:Port（此处的Port是实例的WebSocket服务端口号） |
| `options` | `object` | 否 | - | 初始化选项，支持以下属性（都是可选） |

> **`options` 对象属性：**

| 属性 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `onReady` | `function` | 是 | - | 工程加载成功后的回调函数，只有从这个时候开始，才能调用API， 如果工程尚未打开就调用API，会有崩溃或者错误的风险。 |
| `onLog` | `function` | 是 | - | 用于给DigitalTwinAPI指定输出日志的方法 |
| `onEvent` | `function` | 是 | - | 用于设置事件处理的回调函数，也可以通过setEventCallback方法来设置 |
| `onApiVersion` | `function` | 是 | - | 用于接收云渲染服务器的版本信息的回调方法 |
| `onBeforeRequest` | `function` | 是 | - | 在发送会导致场景修改的接口命令之前执行的回调函数。 在此回调函数可获取到原始的JSON命令字符串。 在此回调函数里，可实现自定义的接口同步操作 |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `url` | `string` | 是 | - | 在渲染程序里运行的HTML页面 |
| `options` | `boolean` | 是 | - | 用于调试的选项。 可设置调试窗口的位置、大小和可见性。 属性如下：x, y, width, height, visible |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `type` | [`ResetType`](/docs/api/types#resettype) | 是 | - | 设置重置的类别，请参考 `ResetType`，如果不传此参数，默认行为是清除添加的所有对象 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ip` | `string` | 是 | - | IP地址或者主机名 |
| `port` | `number` | 是 | - | WebSocket服务的端口号 |
...
| `fn` | `function` | 否 | - | 可选的回调函数，仅在客户端调用时有效。 |
