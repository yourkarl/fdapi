...
![CustomObject 示意图 1](/img/refdoc/api/customobject.gif)
...
- **适用行业**：智慧城市、智慧园区、能源电力、智慧交通、国防、工业、智慧文旅与景区、港口航道
- **使用场景**：
  - 设备、车辆、人员等实体模型的上图与驱动
  - 按实时数据更新模型位姿
  - 模型点选联动业务弹窗与详情
  - 智慧文旅 / 景区：安保人员、车辆、船只实时位置与轨迹驱动，配合 Camera.follow 跟踪。
  - 港口航道 / 海洋：船舶模型实时位置与 AIS 航迹驱动，配合 Camera.follow 跟踪重点船舶。
- **注意事项**：
...
![CustomObject · add(data, fn) 示意](/img/refdoc/api/copy_actor_path.gif)
...
| `pakFilePath` | `string` | 是 | - | pak文件路径，[资源引入说明](/docs/tutorials/resources) |
| `assetPath` | `string` | 是 | - | 蓝图Actor在UE工程的资源引用目录（相对路径），注意：模型必须是蓝图Actor类，即在使用UE打包pak文件时此自定义对象在UE工程内的相对路径 |
| `range` | `array` | 否 | [0, 10000] | 可视范围: [近裁距离, 远裁距离]，默认值: [0, 10000] |
| `location` | `array` | 是 | - | 模型位置坐标：[X,Y,Z]，[取值示例](/docs/tutorials/coordinates)，数组元素类型：(number)，取值范围：[任意数值] |
| `coordinateType` | `number` | 否 | 0 | 坐标系类型，取值范围：0为Projection类型，1为WGS84类型，2为火星坐标系(GCJ02)，3为百度坐标系(BD09)，默认值：0 |
| `rotation` | `array` | 是 | - | 世界坐标系旋转：[Pitch,Yaw,Roll]，数组元素类型：(number)，取值范围：[任意数值] |
| `localRotation` | `array` | 是 | - | 模型自身旋转：[Pitch,Yaw,Roll]，数组元素类型：(number)，取值范围：[任意数值] |
| `isEffectRotation` | `boolean` | 是 | - | 模型是否开启旋转效果，注意：支持startMove()和moveTo()方法调用时生效 |
| `scale` | `array` | 是 | - | 模型缩放：[X,Y,Z]，数组元素类型：(number)，取值范围：[任意正整数] |
| `smoothMotion` | `number` | 是 | - | 模型移动动画效果，1: 平滑移动，0: 跳跃移动 |
| `enableDecal` | `boolean` | 否 | true | 可选，是否支持贴画贴合，默认值：true |
| `visible` | `boolean` | 否 | - | 可选，设置自定义对象加载后是否显示，默认：true |
| `autoHeight` | `boolean` | 否 | false | 可选，是否开启自动高度，默认值：false，注意：开启后自动贴地不再使用模型坐标Z |
| `collision` | `boolean` | 否 | - | 可选，设置自定义对象加载后是否开启碰撞，默认：true |
...
|------|------|:----:|--------|------|
| `id` | `string` | 是 | - | CustomObject对象的ID |
| `tileLayerId` | `string` | 是 | - | TileLayer图层的ID |
| `objectId` | `string \| array` | 是 | - | TileLayer图层中包含的待复制的模型(Actor)的ObjectId，同时也支持数组类型参数即把多个actor复制为一个customObject |
| `location` | `array` | 否 | - | 可选，复制模型的位置坐标：[X,Y,Z]，[取值示例](/docs/tutorials/coordinates)，数组元素类型：(number)，取值范围：[任意数值]，注意：不传此参数会使用原始Actor的位置坐标值 |
| `coordinateType` | `number` | 否 | 0 | 可选，坐标系类型，取值：0为Projection类型，1为WGS84类型，默认值：0 |
| `rotation` | `array` | 否 | - | 可选，复制模型的旋转：[Pitch,Yaw,Roll]，数组元素类型：(number)，取值范围：[任意数值]，注意：不传此参数会使用原始Actor的旋转值 |
| `scale` | `array` | 否 | - | 可选，复制模型的缩放：[X,Y,Z]，数组元素类型：(number)，取值范围：[任意正整数]，注意：不传此参数会使用原始Actor的缩放值 |
| `smoothMotion` | `number` | 否 | 0 | 可选，运动时是否平滑差值，取值范围：1: 平滑插值，0: 跳跃，默认值：0 |
| `visible` | `boolean` | 否 | - | 可选，复制模型完成后是否显示，默认：true |
...
|------|------|:----:|--------|------|
| `id` | `string` | 是 | - | CustomObject对象的ID |
| `hideOriginal` | `boolean` | 否 | true | 可选，是否隐藏被拆分的原始楼宇模型，默认值：true |
| `floors` | `array` | 是 | - | 楼宇模型按楼层高度进行拆分的配置规则数组，每一个数组对象包含的属性如下： |
| `floors.id` | `string` | 是 | - | 拆分后的楼层CustomObject对象的ID |
| `floors.minHeight` | `number` | 是 | - | 当前拆分楼层的底面高度 |
| `floors.maxHeight` | `number` | 是 | - | 当前拆分楼层的顶面高度 |
| `floorMaterial` | `object` | 是 | - | 楼宇模型拆分后的楼层模型的底面材质配置参数： |
| `floorMaterial.material` | `string` | 否 | - | 可选，替换底面材质的文件路径，材质文件对应资源的相对路径 |
| `floorMaterial.scalarParameters` | `array` | 否 | - | 可选，替换底面材质数值类型参数数组，包含name/value键值对的数组，其中value为数值 |
| `floorMaterial.vectorParameters` | `array` | 否 | - | 可选，替换底面材质矢量类型参数数组，包含name/value键值对的数组，其中value为数组 |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | 要删除的CustomObject对象的ID或者ID数组（可以删除一个或者多个） |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | CustomObject对象的ID或者ID数组 |
| `distance` | `number` | 否 | - | 可选参数，观察点距离目标点（被拍摄物体）的距离， 该值有以下3种情况： 1. 如果distance小于0，则是相机跟随移动模式 2. 如果distance等于0，则自动计算飞到合适的距离 3. 如果distance大于0，则是飞到指定距离的位置 |
...
| `actionMode` | [`ActionMode`](/docs/api/types#actionmode) | 否 | - | 可选参数，相机视角跟随模式枚举，注意：如果指定了rotation参数同时又指定了跟随枚举，则枚举值包含的相机欧拉角会覆盖rotation参数，若使用FollowWorldRotation则不覆盖rotation参数 |
| `offset` | `array` | 否 | - | 可选参数，相机视角的偏移量，取值示例：[X, Y, Z]，分别是三个方向的偏移量，单位：米 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | 要获取的CustomObject对象ID或者ID数组（可以获取一个或者多个） |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | customObject对象的id或id数组 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `groupId` | `string` | 是 | - | 自定义对象创建时指定的分组ID |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | CustomObject对象的ID或者数组 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| `rotation` | `array` | 是 | - | 模型旋转：[Pitch,Yaw,Roll]，数组元素类型：(number)，取值范围：[任意数值] |
| `smoothTime` | `-` | 否 | 1 | &#123;number&#125; 可选，平滑移动的插值时间，仅在smoothMotion=1即平滑移动下生效，注意：传值若为0则根据调用setLocation()接口的时间自动计算平滑移动的插值时间，默认值：1，单位：秒 |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `data` | `object \| array` | 是 | - | 替换CustomObject对象材质的详细信息，支持对象或数组，对于每一个对象支持以下属性： |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 属性 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `id` | `string` | 是 | - | 必选参数，CustomObject对象的ID |
| `material` | `string` | 否 | - | 可选参数，替换材质的文件路径，材质文件对应资源的相对路径 |
| `scalarParameters` | `array` | 否 | - | 可选参数，替换材质数值类型参数数组，包含name/value键值对的数组，其中value为数值 |
| `vectorParameters` | `array` | 否 | - | 可选参数，替换材质矢量类型参数数组，包含name/value键值对的数组，其中value为数组 |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | 自定义对象ID或ID数组 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| `smoothTime` | `number` | 否 | 1 | 可选，平滑移动的插值时间，仅在smoothMotion=1即平滑移动下生效，注意：传值若为0则根据调用setLocation()接口的时间自动计算平滑移动的插值时间，默认值：1，单位：秒 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
```js
await fdapi.customObject.setLocation('对象ID', [], 1);
```
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `data` | `object \| array` | 是 | - | 设置自定义对象对应的移动倍速的对象或数组，每个对象包含以下属性： |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
|------|------|:----:|--------|------|
| `id` | `string` | 是 | - | 自定义对象ID |
| `rate` | `number` | 否 | 1（一倍速按设置时间正常移动 | 自定义对象的移动倍速，默认值：1（一倍速按设置时间正常移动），取值范围：[0~任意正数]，2.2即2.2倍速移动，3即3倍速移动，支持小数。 |
...
__co_scale[1] += 0.2;
__co_scale[2] += 0.2;
fdapi.customObject.setScale('o1', [2, 2, 2]);
```
...
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 是 | - | CustomObject对象的ID或者数组 |
| `newColor` | [`Color`](/docs/api/types#color) | 是 | - | 新颜色值 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
|------|------|:----:|--------|------|
| `id` | `string` | 是 | - | CustomObject对象的ID |
| `axis` | [`ForwardAxis`](/docs/api/types#forwardaxis) | 是 | - | (`ForwardAxis`) 自定义模型生长动画轴朝向，取值范围：[0,1,2] 分别对应X、Y、Z三个轴，取值枚举详情参考 `ForwardAxis` |
| `ratio` | `number` | 是 | - | 自定义模型的生长因子，取值范围：[0~1] |
...
|------|------|:----:|--------|------|
| `id` | `string` | 是 | - | CustomObject对象的ID |
| `coordinateType` | `number` | 否 | 0 | 移动轨迹点的坐标系类型，取值范围：0为Projection类型，1为WGS84类型，2为火星坐标系(GCJ02)，3为百度坐标系(BD09)，默认值：0 |
| `pathPointArr` | `array` | 是 | - | 移动轨迹点的坐标数组，其中每个数组元素支持以下属性： |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 属性 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `time` | `number` | 是 | - | 运动到达时间，单位：秒，注意：运动时间起点默认值必须为0 |
| `coordinate` | `array` | 是 | - | 运动轨迹点坐标：[X,Y,Z]，[取值示例](/docs/tutorials/coordinates)，数组元素类型：(number)，取值范围：[任意数值] |
| `rotation` | `array` | 否 | - | 可选参数，每个时刻模型的运动姿态（欧拉角），不传则系统自动计算。格式：[Pitch,Yaw,Roll]，数组元素类型：(number)，取值范围：[任意数值] |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `ids` | `string \| array` | 否 | - | CustomObject对象的ID或者数组 该参数是可选的： - 如果没有指定该参数，则会把场景中所有高亮的CustomObject对消高亮显示 - 如果指定了该参数，则只取消高亮指定的CustomObject对象。 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `data` | `object \| array` | 是 | - | CustomObject对象或者数组，以下属性支持更新 |
| `fn` | `function` | 否 | - | 可选的回调函数，请参考[二次开发：异步接口调用方式](/docs/tutorials/async-call) |
...
| 属性 | 类型 | 必填 | 默认值 | 说明 |
|------|------|:----:|--------|------|
| `id` | `string` | 是 | - | 根据CustomObject对象的ID更新以下属性 |
| `groupId` | `string` | 否 | - | 可选，Group分组 |
| `userData` | `string` | 否 | - | 可选，用户自定义数据 |
| `range` | `array` | 否 | [0, 10000] | 可视范围: [近裁距离, 远裁距离]，默认值: [0, 10000] |
| `location` | `array` | 是 | - | 模型位置坐标：[X,Y,Z]，[取值示例](/docs/tutorials/coordinates)，数组元素类型：(number)，取值范围：[任意数值] |
| `rotation` | `array` | 是 | - | 世界坐标系旋转：[Pitch,Yaw,Roll]，数组元素类型：(number)，取值范围：Pitch[-90~90] Yaw[-180~180] Roll[0] |
| `localRotation` | `array` | 是 | - | 模型自身旋转：[Pitch,Yaw,Roll]，数组元素类型：(number)，取值范围：Pitch[-90~90] Yaw[-180~180] Roll[0] |
| `isEffectRotation` | `boolean` | 是 | - | 模型是否开启旋转效果，注意：支持startMove()和moveTo()方法调用时生效 |
| `scale` | `array` | 是 | - | 模型缩放：[X,Y,Z]，数组元素类型：(number)，取值范围：[任意正整数] |
| `smoothMotion` | `number` | 是 | - | 模型移动动画效果，1: 平滑移动，0: 跳跃移动 |
| `autoHeight` | `boolean` | 否 | false | 可选，是否开启自动高度，默认值：false，注意：开启后自动贴地不再使用模型坐标Z |
